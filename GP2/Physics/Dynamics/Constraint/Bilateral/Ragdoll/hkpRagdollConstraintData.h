...
class hkpRagdollConstraintData : public hkpConstraintData
{
	public:
		...
		hkBool getMotorsActive() const;
			...
			/// Turn the motors on or off ( obsolete, use 'enable motors' )
		...
		void setMotorsActive( hkpConstraintInstance* instance, hkBool toBeEnabled );
			...
			/// \param runtime		The constraint instance's runtime data buffer
			/// \param toBeEnabled	The desired state of the motors.
		struct Runtime;
		void enableMotors( Runtime* runtime, hkBool toBeEnabled );
		...
		enum
		{
			...
			SOLVER_RESULT_MAX = 18 // keep in sync with solver res array below
		};
		...
		struct Runtime
		{
			...
			HK_ALIGN16( class hkpSolverResults m_solverResults[18/*VC6 doesn't like the scoping for SOLVER_RESULT_MAX*/] );
