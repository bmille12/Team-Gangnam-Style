...
class hkpLimitedHingeConstraintData : public hkpConstraintData
{
	public:
		...
		inline hkBool isMotorActive() const;
			...
			/// Turn the motor on or off ( obsolete, switch to the version with Runtime param )
		void setMotorActive( hkpConstraintInstance* instance, hkBool toBeEnabled );

			/// Turn the motor on or off
		struct Runtime;
		void enableMotor( Runtime* runtime, hkBool toBeEnabled );
		...
		struct Runtime
		{
			...
			HK_ALIGN16( class hkpSolverResults m_solverResults[10/*VC6 doesn't like the scoping for SOLVER_RESULT_MAX*/] );
