...
#ifndef HKP_SIMPLE_CONSTRAINT_ATOM_H
#define HKP_SIMPLE_CONSTRAINT_ATOM_H
...
///
/// hkConstraintDatas either use hkpBridgeAtoms, which allow them to implement their custom logic, or use a set of
/// generic hkConstraintAtoms to describe the geometry of the constraint.
///
/// Each hkpConstraintAtom is used to specify a constraint's orientation in space or to create one or more
/// solver-constraints of a given type. During simulation, hkConstraintAtoms are processed in the order in which they
...
struct hkpDeformableLinConstraintAtom : public hkpConstraintAtom
{
	public:
		...
		HK_FORCE_INLINE const hkSymmetricMatrix3&	getYieldStrength() const	{ return m_yieldStrength;				}
		HK_FORCE_INLINE const hkSymmetricMatrix3&	getUltimateStrength() const	{ return m_ultimateStrength;			}
		...
		HK_FORCE_INLINE void setYieldStrength(const hkSymmetricMatrix3& t)		{ m_yieldStrength = t;						}
		HK_FORCE_INLINE void setUltimateStrength(const hkSymmetricMatrix3& t)	{ m_ultimateStrength = t;					}
	...
	public:
		...
		hkSymmetricMatrix3 m_yieldStrength;
		...
		hkSymmetricMatrix3 m_ultimateStrength;
};
...
struct hkpDeformableAngConstraintAtom : public hkpConstraintAtom
{
	public:
		...
		// Getters
		HK_FORCE_INLINE const hkSimdReal			getCosMaxAngle() const		{ return m_yieldStrength.m_diag.getComponent<3>();	}
		HK_FORCE_INLINE const hkQuaternion&			getOffset() const			{ return m_offset;									}
		HK_FORCE_INLINE const hkSymmetricMatrix3&	getYieldStrength() const	{ return m_yieldStrength;							}
		HK_FORCE_INLINE const hkSymmetricMatrix3&	getUltimateStrength() const	{ return m_ultimateStrength;						}

		// Setters
		HK_FORCE_INLINE void setCosMaxAngle(hkSimdRealParameter c)				{ m_yieldStrength.m_diag.setComponent<3>(c);								}
		HK_FORCE_INLINE void setOffset(hkQuaternionParameter q)					{ m_offset = q;																}
		HK_FORCE_INLINE void setYieldStrength(const hkSymmetricMatrix3& t)		{ hkSimdReal c = getCosMaxAngle(); m_yieldStrength = t; setCosMaxAngle(c);	}
		HK_FORCE_INLINE void setUltimateStrength(const hkSymmetricMatrix3& t)	{ m_ultimateStrength = t;													}
...
struct hkpBallSocketConstraintAtom : public hkpConstraintAtom
{
	...
	public:
		...
		{ 
			const int numResults = hkMath::max2<int>(3 * hkpJacobianSchemaInfo::Bilateral1D::Results, hkpJacobianSchemaInfo::StableBallSocket::Results);
			return numResults; 
		}
		...
		{
			const int schemaSize = hkMath::max2<int>(3 * hkpJacobianSchemaInfo::Bilateral1D::Sizeof, hkpJacobianSchemaInfo::StableBallSocket::Sizeof);
			const int numTemps = hkMath::max2<int>(3 * hkpJacobianSchemaInfo::Bilateral1D::Temps, hkpJacobianSchemaInfo::StableBallSocket::Temps);
...
#endif // HKP_SIMPLE_CONSTRAINT_ATOM_H
