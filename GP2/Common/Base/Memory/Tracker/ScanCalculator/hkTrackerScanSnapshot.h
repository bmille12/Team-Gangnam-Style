...
class hkTrackerScanSnapshot: public hkReferencedObject
{
	public:
		...
		struct Block
		{
			...
			hkBool isNamedType(const char* name) const
			{
				...
				/// Start
			Block(const Node* type, const void* start, hk_size_t size, const Allocation* alloc = HK_NULL):
				m_type(type),
			...
			/// Add a block
		Block* addBlock(const Node* type, const void* start, hk_size_t size, const Allocation* alloc = HK_NULL);
	...
	protected:
		...
		hkArray<Allocation> m_allocations;					///< Only leaf-level and inuse allocations (this corresponds to allocations made with the user-level allocators)
