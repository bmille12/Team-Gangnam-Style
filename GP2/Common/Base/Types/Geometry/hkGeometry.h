...
struct hkGeometry
{
	...
	public:
		...
		HK_FORCE_INLINE void appendGeometry(const hkGeometry& geometry, const hkTransform& transform);
		
			/// Append another geometry with optional transform (vertex welding not performed).
		void appendGeometry(const hkGeometry& geometry, const hkMatrix4* transform = HK_NULL);
