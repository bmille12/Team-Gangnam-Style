...
HK_FORCE_INLINE static void apply(hkSimdReal* v, const hkHalf* HK_RESTRICT base)
{
...
template <int M>
template <hkUint32 byteAddressIncrement>
HK_FORCE_INLINE void hkMxReal<M>::gather(const hkHalf* HK_RESTRICT base)
{
	hkMxReal_Implementation::gatherHH<M, byteAddressIncrement>::apply(m_real.r, base);
}
...
namespace hkMxReal_Implementation
{
	...
	template <int I, hkMathAccuracyMode A, hkMathNegSqrtMode D> struct setSqrtInverseHR { HK_FORCE_INLINE static void apply(hkSimdReal* v, const hkSimdReal* v0) { setSqrtInverseHR<I-1,A,D>::apply(v, v0); v[I-1] = v0[I-1].sqrtInverse<A,D>(); } };
	template <hkMathAccuracyMode A, hkMathNegSqrtMode D> struct setSqrtInverseHR<1,A,D> { HK_FORCE_INLINE static void apply(hkSimdReal* v, const hkSimdReal* v0) { v[0] = v0[0].sqrtInverse<A,D>(); } };
}
template <int M>
template <hkMathAccuracyMode A, hkMathDivByZeroMode D> 
...
HK_FORCE_INLINE void hkMxReal<M>::setSqrtInverse(hkMxRealParameter r)
{
	hkMxReal_Implementation::setSqrtInverseHR<M,A,D>::apply(m_real.r, r.m_real.r);
}
...
template <int M>
HK_FORCE_INLINE void hkMxReal<M>::zeroIfTrue(hkMxMaskParameter mask)
{
	hkMxReal<M> zero; zero.setConstant<HK_QUADREAL_0>( );
	setSelect(mask, zero, *this);
}

template <int M>
HK_FORCE_INLINE void hkMxReal<M>::zeroIfFalse(hkMxMaskParameter mask)
{
	hkMxReal<M> zero; zero.setConstant<HK_QUADREAL_0>( );
