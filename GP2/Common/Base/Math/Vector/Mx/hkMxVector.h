...
class hkPackedReal: public hkVector4{ };
...
template <int M> class hkMxQuaternion;
...
public:
	...
	HK_FORCE_INLINE void setAll( hkSimdRealParameter r );

	/// Read subvectors contiguous from memory as abcd efgh ijkl mnop
	HK_FORCE_INLINE void moveLoad(const hkVector4* HK_RESTRICT v);

	template<int N>
	HK_FORCE_INLINE void load(const hkReal* r);	// load N components of a vector sequential in memory SIMD aligned
	...
	template <hkUint32 byteAddressIncrement, int N, hkMathIoMode A> HK_FORCE_INLINE void load(const hkReal* r);
	...
	/// See the documentation at the template values for the requested IO mode.
	template <hkUint32 byteAddressOffset, int N, hkMathIoMode A> HK_FORCE_INLINE void loadWithOffset(const hkReal* r[M]);
	...
	/// See the documentation at the template values for the requested IO mode.
	template <hkUint32 byteAddressIncrement, int N, hkMathIoMode A> HK_FORCE_INLINE void store(hkReal* r) const;
	/// Store N subvector components non-linear to memory using addresses \a base[m] + byteAddressOffset
	/// See the documentation at the template values for the requested IO mode.
	template <hkUint32 byteAddressOffset, int N, hkMathIoMode A> HK_FORCE_INLINE void storeWithOffset(hkReal* r[M]) const;
	...
	template <hkUint32 byteAddressIncrement, int N> HK_FORCE_INLINE void gather(const hkVector4* base);
	...
	template <hkUint32 byteAddressIncrement, int N> HK_FORCE_INLINE void scatter(hkVector4* base) const;
	...
	HK_FORCE_INLINE void setMul(hkMxRealParameter v, hkMxSingleParameter s);
	...
	HK_FORCE_INLINE void setRotatedDir(hkMxQuaternionParameter v0, hkMxVectorParameter v1);
	HK_FORCE_INLINE void setRotatedInverseDir(hkMxQuaternionParameter v0, hkMxVectorParameter v1);
	...
	HK_FORCE_INLINE void subMul(hkMxVectorParameter v0, hkMxVectorParameter v1);
	HK_FORCE_INLINE void subMul(hkMxVectorParameter v, hkMxSingleParameter s);
	HK_FORCE_INLINE void subMul(hkMxSingleParameter v, hkMxSingleParameter s);
	HK_FORCE_INLINE void subMul(hkMxRealParameter v, hkMxVectorParameter s);
	...
	template <hkMxVectorPermutation::Permutation P> HK_FORCE_INLINE void setVectorPermutation(hkMxVectorParameter v);

	/// Set each compenent of self to a permutation of \a v. 
	template <hkVectorPermutation::Permutation P> HK_FORCE_INLINE void setComponentPermutation(hkMxVectorParameter v);
	...
	/// \a addsOut = xyzw with x=a+b+c+d, y=e+f+g+h, z=i+j+k+l, w=m+n+o+p
	/// \warning [M:1 only] 
	template <int N> HK_FORCE_INLINE void horizontalAdd( hkSimdReal& addsOut ) const; 
	...
	HK_FORCE_INLINE void setAll( const hkPackedReal& v ){ setAsBroadcast( v ); }
