...
public:
	...
	template <hkMathAccuracyMode A, hkMathNegSqrtMode S> HK_FORCE_INLINE void setSqrtInverse(hkMxRealParameter r);

	// Zero I-th scalar if the corresponding mask is set.
	HK_FORCE_INLINE void zeroIfTrue(hkMxMaskParameter mask);
	// Zero I-th scalar if the corresponding mask is not set.
	HK_FORCE_INLINE void zeroIfFalse(hkMxMaskParameter mask);
