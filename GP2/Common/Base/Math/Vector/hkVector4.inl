...
HK_FORCE_INLINE hkVector4::hkVector4( const VVector3& v) 
{ 
	load<3,HK_IO_NATIVE_ALIGNED>( (hkReal*)&v ); 
	zeroComponent<3>(); 
}
#endif
#if !defined(HK_DISABLE_IMPLICIT_VVECTOR3_CONVERSION)
HK_FORCE_INLINE hkVector4::operator const VVector3&() const 
{ 
	return (const VVector3&)*this; 
}
#endif
...
{
	return equal(v).allAreSet<hkVector4Comparison::MASK_XY>();
}

template <>
HK_FORCE_INLINE hkBool32 hkVector4::allExactlyEqual<3>(hkVector4Parameter v) const
{
	return equal(v).allAreSet<hkVector4Comparison::MASK_XYZ>();
}
...
template <>
HK_FORCE_INLINE hkBool32 hkVector4::allComponentsEqual<4>() const
{
	hkVector4 yxwz; yxwz.setPermutation<hkVectorPermutation::YXWZ>(*this);
	const hkVector4Comparison xz_eq_yw = equal(yxwz);

	hkVector4 zwxy; zwxy.setPermutation<hkVectorPermutation::ZWXY>(*this);
	const hkVector4Comparison xy_eq_zw = equal(zwxy);

	hkVector4Comparison all_eq; all_eq.setAnd(xz_eq_yw, xy_eq_zw);
	return all_eq.allAreSet();
}

template <>
HK_FORCE_INLINE hkBool32 hkVector4::allComponentsEqual<3>() const
{
	hkVector4 yxzw; yxzw.setPermutation<hkVectorPermutation::YXZW>(*this);
	const hkVector4Comparison x_eq_y = equal(yxzw);

	hkVector4 zwxy; zwxy.setPermutation<hkVectorPermutation::ZWXY>(*this);
	const hkVector4Comparison x_eq_z = equal(zwxy);

	hkVector4Comparison x_eq_y_eq_z; x_eq_y_eq_z.setAnd(x_eq_y, x_eq_z);
	return x_eq_y_eq_z.allAreSet<hkVector4Comparison::MASK_X>();
}

template <>
HK_FORCE_INLINE hkBool32 hkVector4::allComponentsEqual<2>() const
{
	hkVector4 yx; yx.setPermutation<hkVectorPermutation::YXZW>(*this);
	return allExactlyEqual<2>(yx);
}

template <>
HK_FORCE_INLINE hkBool32 hkVector4::allComponentsEqual<1>() const
{
	return true;
}

template <int N>
HK_FORCE_INLINE hkBool32 hkVector4::allComponentsEqual() const
{
	HK_VECTOR4_NOT_IMPLEMENTED;
	return false;
}
...
{
	return less(a).allAreSet<hkVector4Comparison::MASK_XY>();
}

template <>
HK_FORCE_INLINE hkBool32 hkVector4::allLess<3>(hkVector4Parameter a) const
{
	return less(a).allAreSet<hkVector4Comparison::MASK_XYZ>();
}
...
{
	return lessZero().allAreSet<hkVector4Comparison::MASK_XY>();
}

template <>
HK_FORCE_INLINE hkBool32 hkVector4::allLessZero<3>() const
{
	return lessZero().allAreSet<hkVector4Comparison::MASK_XYZ>();
}
...
template <int I> 
HK_FORCE_INLINE const hkReal* hkVector4::getComponentAddress() const
{
...
template <int I> 
HK_FORCE_INLINE hkReal* hkVector4::getComponentAddress()
{
	HK_VECTOR4_SUBINDEX_CHECK;
	return ((hkReal*)&m_quad) + I;
}

HK_FORCE_INLINE const hkReal* hkVector4::getComponentAddress(int i) const
{
	HK_MATH_ASSERT(0x6d0431d7, i>=0 && i<4, "index out of bounds for component access");
	return ((const hkReal*)&m_quad) + i;
}

HK_FORCE_INLINE hkReal* hkVector4::getComponentAddress(int i)
{
	HK_MATH_ASSERT(0x6d0431d7, i>=0 && i<4, "index out of bounds for component access");
	return ((hkReal*)&m_quad) + i;
}
...
{
#if (HK_CONFIG_SIMD == HK_CONFIG_SIMD_ENABLED)
	hkVector4 ma; ma.setHorizontalMax<3>( *this );
	hkVector4Comparison isMax = greaterEqual(ma);
	hkVector4Comparison xyzMask; xyzMask.set<hkVector4Comparison::MASK_XYZ>();
	isMax.setAnd(isMax,xyzMask);
	HK_ON_DEBUG(const int mask = isMax.getMask();)
	HK_MATH_ASSERT(0x2842fb1, mask > 0, "inconsistent max value of self");
	HK_MATH_ASSERT(0x2842fb2, (mask & hkVector4Comparison::MASK_W) == 0, "selective compare failed");
	return isMax.getIndexOfLastComponentSet();
#else
	const hkReal* HK_RESTRICT tmp = (const hkReal* HK_RESTRICT)this;
...
{
#if (HK_CONFIG_SIMD == HK_CONFIG_SIMD_ENABLED)
	hkVector4 mi; mi.setHorizontalMin<3>( *this );
	hkVector4Comparison isMin = lessEqual(mi);
	hkVector4Comparison xyzMask; xyzMask.set<hkVector4Comparison::MASK_XYZ>();
...
template<int vectorConstant>
HK_FORCE_INLINE void hkVector4::setConstant()
{
...
HK_FORCE_INLINE void hkVector4::setClampedToMaxLength(hkVector4Parameter vSrc, hkSimdRealParameter constMaxLen)
{
	...
	hkSimdReal maxLen; maxLen.setMax(constMaxLen, hkSimdReal_Eps);

	// maxLen can be big, i.e. HK_REAL_MAX, so we can't square it
	hkSimdReal len = vSrc.length<3>();
	hkSimdReal invLen; invLen.setReciprocal(len);

	// calc rescaled
	hkVector4 rescaledSrc;
...
HK_FORCE_INLINE hkBool hkVector4::isOk3() const { return (hkBool)isOk<3>(); }
HK_FORCE_INLINE hkBool hkVector4::isOk4() const { return (hkBool)isOk<4>(); }
