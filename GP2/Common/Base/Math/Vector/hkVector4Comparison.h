...
{
	template <bool b> struct HK_VECTORCOMPARISON_ILLEGAL_MASK_VALUE;
	template <> struct HK_VECTORCOMPARISON_ILLEGAL_MASK_VALUE<true> {};

	template <bool b> struct HK_VECTORCOMPARISON_SUBINDEX_OUT_OF_RANGE;
	template <> struct HK_VECTORCOMPARISON_SUBINDEX_OUT_OF_RANGE<true> {};
}
#define HK_VECTORCOMPARISON_MASK_CHECK           HK_COMPILE_TIME_ASSERT2((M&0xf)==M, HK_VECTORCOMPARISON_ILLEGAL_MASK_VALUE)
#define HK_VECTORCOMPARISON_SUBINDEX_CHECK		 HK_COMPILE_TIME_ASSERT2((N>0)&&(N<=4), HK_VECTORCOMPARISON_SUBINDEX_OUT_OF_RANGE);
...
public:
	...
	HK_FORCE_INLINE void setXor( hkVector4ComparisonParameter a, hkVector4ComparisonParameter b );
	...
	template <int N>
	HK_FORCE_INLINE const hkVector4Comparison horizontalOr() const;
