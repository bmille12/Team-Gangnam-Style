...
class hkIntVector
{
	public:
		...
		/// cast a vector to an int vector. If the vector is double, convert it to float first.
		HK_FORCE_INLINE void loadAsFloat32BitRepresentation(const hkVector4& x);

		/// interpret an int vector as a float32 vector and convert it to hkReal[4]
		HK_FORCE_INLINE void storeAsFloat32BitRepresentation( hkVector4& out) const;

		/// This stores the ints in the form used by hkVector4::getInt24W()
		HK_FORCE_INLINE void storeInto24LowerBitsOfReal(hkVector4& vOut);
