...
{
	const hkReal fa = a.getReal();
	const hkReal fb = b.getReal();
	const hkReal fc = c.getReal();
	const hkReal fd = d.getReal();
	out.set(fa,fb,fc,fd);
}
#endif
...
HK_FORCE_INLINE void HK_CALL hkVector4Util::calculatePerpendicularVector(hkVector4Parameter vectorIn, hkVector4& biVectorOut)
{
	...
	hkVector4 h; h.setXYZ_0( vectorIn );
	hkVector4 d0; d0.setPermutation<hkVectorPermutation::YXWW>(h);
	hkVector4 d1; d1.setPermutation<hkVectorPermutation::ZWXW>(h);
	const hkSimdReal l0 = d0.lengthSquared<3>();
	const hkSimdReal l1 = d1.lengthSquared<3>();
	const hkVector4Comparison c = l0.less( l1 );
	hkVector4 out; out.setSelect(c,d1,d0);
	biVectorOut.setNeg<1>(out);
#else
...
#ifndef HK_VECTOR4UTIL_calculatePerpendicularNormalizedVectors
template<bool INVERSE2>
HK_FORCE_INLINE void HK_CALL hkVector4Util::calculatePerpendicularNormalizedVectors(hkVector4Parameter vectorIn, hkVector4& biVectorOut, hkVector4& bi2VectorOut)
{
	HK_ASSERT( 0xf034fd5f, vectorIn.isNormalized<3>(1e-3f) );
#if (HK_CONFIG_SIMD == HK_CONFIG_SIMD_ENABLED)
	hkVector4 h; h.setXYZ_0( vectorIn );
	hkVector4 d0; d0.setPermutation<hkVectorPermutation::YXWW>(h);
	hkVector4 d1; d1.setPermutation<hkVectorPermutation::ZWXW>(h);
	const hkSimdReal l0 = d0.lengthSquared<3>();
	const hkSimdReal l1 = d1.lengthSquared<3>();
	hkSimdReal len; len.setMax( l0, l1 );
	const hkVector4Comparison c = l0.less( l1 );
	hkVector4 o0; o0.setSelect(c,d1,d0);
	o0.setNeg<1>(o0);
	hkVector4 o1;
	if ( INVERSE2	)
	{
	...
	hkSimdReal invLen = len.sqrtInverse<HK_ACC_23_BIT, HK_SQRT_IGNORE>(  );
	...
	biVectorOut(ok1) =  vectorIn(ok2);
	biVectorOut(ok2) = -vectorIn(ok1);
	biVectorOut.normalize<3,HK_ACC_23_BIT, HK_SQRT_IGNORE>();
...
HK_FORCE_INLINE void HK_CALL hkVector4Util::transformPlaneEquations( const hkTransform& t, const hkVector4* HK_RESTRICT planes, int numPlanes, hkVector4* HK_RESTRICT planesOut )
{
	...
	do
	{
		...
		planesOut->_setRotatedDir(rotation, planes[0] );
...
HK_FORCE_INLINE void HK_CALL hkVector4Util::transformPlane( const hkTransform& transform, const hkVector4& scale, hkVector4& planeInOut )
{
	...
	//
	//    T is hkQsTransform, and Tn is the inverse transpose of RS, used
	...
	const hkSimdReal w = planeInOut.getW();
	const hkVector4 translation = transform.getTranslation();
	hkVector4 invScale; invScale.setReciprocal(scale); 
	hkVector4 planeLocal; planeLocal.setMul(planeInOut, invScale); 
	planeLocal._setRotatedDir(transform.getRotation(), planeLocal);
	const hkSimdReal invNorm = planeLocal.lengthInverse<3,HK_ACC_23_BIT,HK_SQRT_SET_ZERO>();
	const hkSimdReal dot = planeLocal.dot<3>(translation);
	planeLocal.setW( w-dot );
	planeLocal.mul(invNorm); // notice w must be scaled as well
	planeInOut = planeLocal;
}

HK_FORCE_INLINE void HK_CALL hkVector4Util::scalePlane( const hkVector4& scale, hkVector4& planeInOut )
{
	// By setting R = identity in the derivation in hkVector4Util::transformPlane(),
...
HK_FORCE_INLINE void HK_CALL hkVector4Util::translatePlane( const hkVector4& translation, hkVector4& planeInOut )
{
	// By setting S = R = identity in the derivation in hkVector4Util::transformPlane(),
	hkVector4 planeLocal = planeInOut; 
	const hkSimdReal w = planeLocal.getW();
	const hkSimdReal dot = planeLocal.dot<3>(translation);
	planeLocal.setW( w-dot );
	planeInOut = planeLocal;
}
...
#ifndef HK_VECTOR4UTIL_dot3_1vs3
HK_FORCE_INLINE void HK_CALL hkVector4Util::dot3_1vs3(hkVector4Parameter vectorIn, hkVector4Parameter a0, hkVector4Parameter a1, hkVector4Parameter a2, hkVector4& dotsOut)
{
#if (HK_CONFIG_SIMD == HK_CONFIG_SIMD_ENABLED)
	hkVector4 c0; c0.setMul(a0, vectorIn);
	hkVector4 c1; c1.setMul(a1, vectorIn);
...
#endif	//	HK_VECTOR4UTIL_dot3_1vs3
...
#ifndef HK_VECTOR4UTIL_dot4_1vs3
HK_FORCE_INLINE void HK_CALL hkVector4Util::dot4_1vs3( hkVector4Parameter vectorIn, hkVector4Parameter a0, hkVector4Parameter a1, hkVector4Parameter a2, hkVector4& dotsOut)
{
	...
	dotsOut.add(s);
#else
	dotsOut.set( a0.dot<4>( vectorIn ), a1.dot<4>( vectorIn ), a2.dot<4>( vectorIn ), hkSimdReal_0 );
#endif
}
#endif	// HK_VECTOR4UTIL_dot4_1vs3

#ifndef HK_VECTOR4UTIL_dot4xyz1_1vs4
HK_FORCE_INLINE void HK_CALL hkVector4Util::dot4xyz1_1vs4( hkVector4Parameter vectorIn, hkVector4Parameter a0, hkVector4Parameter a1, hkVector4Parameter a2, hkVector4Parameter a3, hkVector4& dotsOut)
{
#if (HK_CONFIG_SIMD == HK_CONFIG_SIMD_ENABLED)
	hkVector4 c0; c0.setMul(a0, vectorIn);
	hkVector4 c1; c1.setMul(a1, vectorIn);
	...
	HK_TRANSPOSE4(c0, c1, c2, c3);
	hkVector4 w; w.setAll(vectorIn.getW());
	hkVector4 s; s.setAdd(c2, w);
	dotsOut.setAdd(c0, c1);
	dotsOut.add(s);
#else
	dotsOut.set( a0.dot4xyz1( vectorIn ),
		a1.dot4xyz1( vectorIn ),
		a2.dot4xyz1( vectorIn ),
		a3.dot4xyz1( vectorIn ) );
#endif
}
#endif
...
#ifndef HK_VECTOR4UTIL_broadCastAtVectorMax
HK_FORCE_INLINE void HK_CALL hkVector4Util::setXAtVectorMax( hkVector4Parameter check, hkIntVector& iVinOut, hkVector4& v0InOut, hkVector4& v1InOut)
{
#if (HK_CONFIG_SIMD == HK_CONFIG_SIMD_ENABLED) && !defined(HK_USING_GENERIC_INT_VECTOR_IMPLEMENTATION)
	hkVector4 zw; zw.setPermutation<hkVectorPermutation::ZWZW>( check );
	hkVector4Comparison zwGreater = zw.greater( check );

	zw.setMax( zw, check );
	hkIntVector ivXY, ivZW;   ivZW.setPermutation<hkVectorPermutation::ZWZW>( iVinOut ); ivXY.setSelect( zwGreater, ivZW, iVinOut );
	hkVector4   v0XY, v0ZW;   v0ZW.setPermutation<hkVectorPermutation::ZWZW>( v0InOut ); v0XY.setSelect( zwGreater, v0ZW, v0InOut );
	hkVector4   v1XY, v1ZW;   v1ZW.setPermutation<hkVectorPermutation::ZWZW>( v1InOut ); v1XY.setSelect( zwGreater, v1ZW, v1InOut );

	hkVector4 y; y.setPermutation<hkVectorPermutation::YYYY>( zw );
	hkVector4Comparison yGreater = y.greater( zw );

	hkIntVector ivY; ivY.setPermutation<hkVectorPermutation::YYYY>( ivXY ); iVinOut.setSelect( yGreater, ivY, ivXY );
	hkVector4 v0Y;   v0Y.setPermutation<hkVectorPermutation::YYYY>( v0XY ); v0InOut.setSelect( yGreater, v0Y, v0XY );
	hkVector4 v1Y;   v1Y.setPermutation<hkVectorPermutation::YYYY>( v1XY ); v1InOut.setSelect( yGreater, v1Y, v1XY );
...
#ifndef HK_VECTOR4UTIL_setPermutation2
template<unsigned int i, unsigned int j, unsigned int k, unsigned int l>
HK_FORCE_INLINE void HK_CALL hkVector4Util::setPermutation2(hkVector4Parameter a, hkVector4Parameter b, hkVector4 & out)
{
	HK_ASSERT(0x72c2a180, i<8 && j<8 && k<8 && l<8);
...
#define HK_VECTOR4UTIL_SELECT_MASK \
	hkVector4Comparison::Mask( \
		  ((i & 0x4) ? 0 : hkVector4Comparison::MASK_X) \
		| ((j & 0x4) ? 0 : hkVector4Comparison::MASK_Y) \
		| ((k & 0x4) ? 0 : hkVector4Comparison::MASK_Z) \
		| ((l & 0x4) ? 0 : hkVector4Comparison::MASK_W))

	hkVector4 aPerm; aPerm.setPermutation<HK_VECTOR4UTIL_PERM_MASK>(a);
	hkVector4 bPerm; bPerm.setPermutation<HK_VECTOR4UTIL_PERM_MASK>(b);
