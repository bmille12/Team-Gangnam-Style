...
{
#if (HK_CONFIG_SIMD == HK_CONFIG_SIMD_ENABLED)
	const hkVector4* HK_RESTRICT id = (const hkVector4* HK_RESTRICT)(g_vectorConstants + HK_QUADREAL_1000);
...
HK_FORCE_INLINE hkBool32 hkMatrix4::isApproximatelyIdentity( hkSimdRealParameter epsilon) const
{
	hkVector4 epsilon_v; epsilon_v.setAll(epsilon);
	const hkVector4* HK_RESTRICT id = (const hkVector4* HK_RESTRICT)(g_vectorConstants + HK_QUADREAL_1000);

	hkVector4 t0; t0.setSub(m_col0, id[0]);	t0.setAbs( t0 );
	hkVector4 t1; t1.setSub(m_col1, id[1]);	t1.setAbs( t1 );
	hkVector4 t2; t2.setSub(m_col2, id[2]);	t2.setAbs( t2 );
	hkVector4 t3; t3.setSub(m_col3, id[3]);	t3.setAbs( t3 );
