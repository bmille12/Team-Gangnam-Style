...
{	
	public:
		...
		/// Grows the array by the specified quantity. New elements are not constructed, instead OPERATIONS::setEmtpy is called 
		/// on them.
		...
		HK_FORCE_INLINE	INDEX_TYPE indexOf(const VALUE_TYPE& element) const;
	...
	private:
		...
		class OperationsImplementIsEmpty
		{
			protected:
				...
				typedef hkBool32 (*IsEmptyPtr) (const VALUE_TYPE&);
