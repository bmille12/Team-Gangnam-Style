...
class hkDebugDisplayHandler
{
	...
	public:
		...
		virtual hkResult displayPoint2d(const hkVector4& position, int colour, int id, int tag) = 0;

			/// Puts a display line in 2D [-1..1] normalized windows coordinates into the display buffer for display in the next frame.
		virtual hkResult displayLine2d(const hkVector4& start, const hkVector4& end, int colour, int id, int tag) = 0;

			/// Puts a display triangle in 2D [-1..1] normalized windows coordinates into the display buffer for display in the next frame.
		virtual hkResult displayTriangle2d(const hkVector4& a, const hkVector4& b, const hkVector4& c, int colour, int id, int tag) = 0;

			/// Outputs text in 2D [-1..1] normalized windows coordinates. Also scales the text wrt the font size using sizeScale.
		virtual hkResult displayText2d(const char* text, const hkVector4& pos, hkReal sizeScale, int color, int id, int tag) = 0;
