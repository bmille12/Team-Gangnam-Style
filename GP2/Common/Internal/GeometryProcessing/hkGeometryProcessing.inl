...
inline hkVector4	hkGeometryProcessing::minimizeN_R(int steps, int maxDepth, hkReal minConvergence, const FUNCTION& function)
{
	...
	{
		fractions[i].setFromFloat(hkReal(i) / hkReal(fractions.getSize()-1));
	}
	...
	{
		hfractions[i] = (fractions[i] + fractions[i+1]) * hkSimdReal::getConstant<HK_QUADREAL_INV_2>();
	}
	...
	hkVector4	solution; solution.setZero(); solution(3) = HK_REAL_MAX;
	for(int depth = 0; depth < maxDepth; ++depth)
	{
		const hkSimdReal	xDelta = xSpan[1] - xSpan[0];
		const hkSimdReal	yDelta = ySpan[1] - ySpan[0];
		const hkReal		prevValue = solution(3);
		int					bestX=-1,bestY=-1;
		...
		for(int y = 0; y < hfractions.getSize(); ++y)
		{
			...
			for(int x = 0; x < hfractions.getSize(); ++x)
			{
				...
				if(bestX == -1 || value < solution.getComponent<3>())
				{
		...
		const hkReal convergence = prevValue - solution(3);

		if(convergence > minConvergence && bestX != -1)
		{
