	...
	)
{
	...
	while(true)
	{
		...
		// Check for not colliding
		{
			...
			{
				return 0;	// At least one of the planes is not colliding
			}
		}
	...
	// Final checks
	{
		...
		// Set normal
		{
			...
			normalOut->setFlipSign( planeEquations[bestNrmIndex], isInside );
		}

		// no inside hits: (lambda1 < lambda0) || (lambda0 < 0) means no intersection
		hkVector4Comparison noInter0; noInter0.setOr(lambda1.less(lambda0), lambda0.lessZero());
		// inside hits: lambda1 was never set means no intersection
		hkVector4Comparison noInter1 = lambda1.equal( hkSimdReal_2);

		hkVector4Comparison noInter; noInter.setSelect(pathSelect, noInter1, noInter0);
		return !noInter.anyIsSet();
	}
}
